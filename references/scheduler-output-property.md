...
bash "$SKILL/scripts/ds_studio_bridge.sh" click-text 'Data Output Setting'
...
bash "$SKILL/scripts/ds_studio_bridge.sh" set-field-by-label 'Output name' 'my_output'
...
bash "$SKILL/scripts/ds_studio_bridge.sh" toggle-by-label 'Auto Retry' on
```
...
4. Configure switches:
   ```bash
   bash "$SKILL/scripts/ds_studio_bridge.sh" toggle-by-label 'Auto Retry' on
   bash "$SKILL/scripts/ds_studio_bridge.sh" toggle-by-label 'Latest Only' off
   bash "$SKILL/scripts/ds_studio_bridge.sh" toggle-by-label 'Email Upon Successful Completion' off
   ```
